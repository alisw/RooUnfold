...
RooUnfoldResponse::RooUnfoldResponse(const TH1* measured, const TH1* truth, const TH1* fakes, const TH2* response,
		const char* name, const char* title) :
		TNamed(name, title) {
	// RooUnfoldResponse constructor - create from already-filled histograms
	// "response" gives the response matrix, measured X truth.
	...
	// to indicate, respectively, no fakes and/or no inefficiency.
	Init();
	Setup(measured, truth, fakes, response);
}
...
void RooUnfoldResponse::ReplaceAxis(TAxis* dest, const TAxis* source) {
...
RooUnfoldResponse::Setup(const TH1* measured, const TH1* truth) {
	...
	if (_mdim == 1)
		ReplaceAxis(_res->GetXaxis(), _mes->GetXaxis());
	if (_tdim == 1)
		ReplaceAxis(_res->GetYaxis(), _tru->GetXaxis());
	TH1::AddDirectory(oldstat);
	return *this;
}
...
RooUnfoldResponse::Setup(const TH1* measured, const TH1* truth, const TH2* response) {
	...
	if (_nm != _res->GetNbinsX() || _nt != _res->GetNbinsY()) {
		cerr << "Warning: RooUnfoldResponse measured X truth is " << _nm << " X " << _nt << ", but matrix is "
				<< _res->GetNbinsX() << " X " << _res->GetNbinsY() << endl;
	}
	...
	if (!measured || _mes->GetEntries() == 0.0) {
		...
		for (Int_t i = 0; i < nm; i++) {
			...
			for (Int_t j = 0; j < _nt + 2; j++) {
				...
				if (s)
					wmes += pow(_res->GetBinError(i + first, j), 2);
			}
	...
	if (!truth || _tru->GetEntries() == 0.0) {
		...
		for (Int_t j = 0; j < nt; j++) {
			...
			for (Int_t i = 0; i < _nm + 2; i++) {
				...
				if (s)
					wtru += pow(_res->GetBinError(i, j + first), 2);
			}
			Int_t bin = GetBin(_tru, j, _overflow);
			_tru->SetBinContent(bin, ntru);
			if (s)
				_tru->SetBinError(bin, sqrt(wtru));
		}
	}
...
RooUnfoldResponse&
RooUnfoldResponse::Setup(const TH1* measured, const TH1* truth, const TH1* fakes, const TH2* response) {
	// Set up from already-filled histograms.
	// "response" gives the response matrix, measured X truth.
	// "measured" and "truth" give the projections of "response" onto the X-axis and Y-axis respectively,
	// but with additional entries in "measured" for measurements with no corresponding truth (fakes/background) and
...
//		_fak->SetTitle("Fakes");
	...
	if (fakes) {
		_fak = (TH1*) fakes->Clone();
		assert(_mdim == _fak->GetDimension());
	} else {
		_fak = new TH1D("fakes", "fakes", response->GetNbinsX(), 0.0, 1.0);
		ReplaceAxis(_fak->GetXaxis(), _res->GetXaxis());
	}
	TH1::AddDirectory(oldstat);
	if (_overflow && (_mdim > 1 || _tdim > 1)) {
		cerr << "UseOverflow setting ignored for multi-dimensional distributions" << endl;
...
void RooUnfoldResponse::ClearCache() {
	delete _vMes;
	_vMes = 0;
	delete _eMes;
	_eMes = 0;
...
Int_t RooUnfoldResponse::Fill(Double_t xr, Double_t yr, Double_t zr, Double_t xt, Double_t yt, Double_t zt,
		Double_t w) {
...
Int_t RooUnfoldResponse::GetBinDim(const TH1* h, Int_t i) {
...
RooUnfoldResponse::H2H1D(const TH1* h, Int_t nb) {
	if (dynamic_cast<const TH1D*>(h))
		return dynamic_cast<TH1D*>(h->Clone());
...
void RooUnfoldResponse::V2H(const TVectorD& v, TH1* h, Int_t nb, Bool_t overflow) {
...
RooUnfoldResponse::ApplyToTruth(const TH1* truth, const char* name) const {
	...
	TVectorD* resultvect;
	if (truth) {
		if (truth->GetNbinsX() != _tru->GetNbinsX() || truth->GetNbinsY() != _tru->GetNbinsY()
				|| truth->GetNbinsZ() != _tru->GetNbinsZ())
			cerr << "Warning: RooUnfoldResponse::ApplyToTruth truth histogram is a different size ("
					<< (truth->GetNbinsX() * truth->GetNbinsY() * truth->GetNbinsZ())
...
void RooUnfoldResponse::SetNameTitleDefault(const char* defname, const char* deftitle) {
